...
1. THE system SHALL create ExternalSecret resource for PostgreSQL credentials
2. THE system SHALL create ExternalSecret resource for Dragonfly credentials
3. THE system SHALL create ExternalSecret resource for LLM API keys
4. THE ExternalSecrets SHALL reference the ClusterSecretStore aws-parameter-store
5. THE ExternalSecrets SHALL be in platform/claims/intelligence-deepagents/external-secrets/ directory
...
**User Story:** As a backend developer, I want ImagePullSecret configured for private registry access, so that Kubernetes can pull the agent_executor image.
...
3. THE claim SHALL reference ghcr-pull-secret in imagePullSecrets field
4. THE ImagePullSecret SHALL contain valid GitHub token with package read permissions
5. THE ImagePullSecret SHALL be in platform/claims/intelligence-deepagents/image-pull-secret.yaml
...
**User Story:** As a backend developer, I want AWS SSM Parameter Store configured with secrets, so that ESO can sync them to Kubernetes.

#### Acceptance Criteria

1. THE AWS SSM Parameter Store SHALL contain PostgreSQL credentials at /zerotouch/prod/agent-executor/postgres/*
2. THE AWS SSM Parameter Store SHALL contain Dragonfly credentials at /zerotouch/prod/agent-executor/dragonfly/*
3. THE AWS SSM Parameter Store SHALL contain LLM API keys at /zerotouch/prod/agent-executor/openai_api_key and anthropic_api_key
4. THE parameters SHALL use SecureString type for sensitive values
5. THE ESO SHALL have IAM permissions to read these parameters
